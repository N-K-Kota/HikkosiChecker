...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func initData()->List<Sectionobj>{
        let task1 = Task()
        task1.task = "持ち物の分類"
        task1.point = "持って行くもの、売却するもの、捨てるものを分けましょう"
        
        let task2 = Task()
        task2.task = "粗大ゴミの申し込み"
        task2.task = "市区町村によってゴミの出し方、処分費用は変わるので確認しましょう。\n粗大ゴミは月一回しか出せない市区町村も多いので早めに出せる日にちを確認するのことが必要です。\nクリーンセンターなどに持ち込みをすると費用が抑えられることもありますが、その場合も予約が必要です。"
        let task3 = Task()
...
また、照明のコンセントの形によってつけられる照明も決まってきます。
◎室内の汚れ、傷はあるか
入居後傷を発見した場合は修繕費用の負担を求められることになることがあるので注意が必要です。
☆採寸、撮影
下見の際は後々家具を購入、搬入する際に困らないよう、しっかりと採寸をしておきましょう。
採寸するべき箇所は以下の通りです
◎通路、ドアの横幅
◎洗濯槽の設置場所の大きさ
◎キッチンのシンクの大きさ
...
""",nil,false])
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions:
        ...
        let realm = try! Realm()
        if(UserDefaults.standard.object(forKey: "flagtest") != nil){
            checkedObj = realm.objects(CheckedObj.self).first!
            uncheckedObj = realm.objects(UncheckedObj.self).first!
        }else{
            print("noflag")
            UserDefaults.standard.set(true, forKey: "flagtest")
